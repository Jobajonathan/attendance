...
export default async function ReviewPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  // The very first visitor to a review link "wakes up" a stale scheduled/open
  // status, independent of staff activity or the coarse daily cron.
  await supabase.rpc("sync_activity_statuses");

  const { data: activityRows } = await supabase.rpc("get_review_activity", { p_link_token: token });
      ...
      <Card className="w-full max-w-sm p-8">
        {!activity ? (
          ...
          <>
            <h1 className="font-heading text-lg font-semibold text-neutral-900">{activity.title}</h1>
            <Alert tone="info" className="mt-3">
              This review hasn&apos;t opened yet. It opens at{" "}
          ...
          <>
            <h1 className="font-heading text-lg font-semibold text-neutral-900">{activity.title}</h1>
            <p className="mt-1 text-sm text-neutral-500">Enter the keyword you were given to submit.</p>
            <ReviewForm token={token} members={members} />
          </>
        )}
      </Card>
    </div>
